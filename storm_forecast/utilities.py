...
def feature_groups(X_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    """
    X_0D = pd.concat([X_df['instant_t', 'windspeed', 'latitude', 'longitude',
    ...
    X_zuv = X_df[[col for col in X_df.columns if col.startswith('z_') or col.startswith('u_') or col.startswith('v_')]]
    X_sshv = X_df[[col for col in X_df.columns if col.startswith('sst') or col.startswith('slp') \\
